...
//                damage();
    ...
    private static void makeBlockChain() {
        ...
        for (int i = 1; i <= LENGTH; i++) {
            ...
            try {
                ...
                blockInfo.setSign(Utils.generateRSAPSSSignature(keys.getPrivate(), prevHash));
                blockInfo.setSign2(Utils.generateRSAPSSSignature2(keys.getPrivate(), blockInfo.getData()));
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private static boolean verification() throws GeneralSecurityException, UnsupportedEncodingException {
        ...
        for (int i = 1; i < LENGTH; i++) {
            ...
            if (!Utils.verifyRSAPSSSignature(keys.getPublic(), prevHash, blockchain.get(i).getSign())) {
                return false;
            }
            if (!Utils.verifyRSAPSSSignature(keys.getPublic(), data.getBytes(StandardCharsets.UTF_8), blockchain.get(i).getSign2())) {
                return false;
            }
        }
